...
### 3.8 运行模式体系 (默认全功能 vs --demo) 与成果输出 (L950 - L1107) ★重要更新
* **命令行参数体系（CLI）**：
  * `python P2.py`：**默认全功能高精度求解模式**（20m/100m/25m，耗时约 1~2 分钟，直接用于论文最终结论与出图 `P2_result.png`）；
  * `python P2.py --demo`：**快速演示模式**（60m/200m/50m，耗时约 5 秒，用于快速代码调试和连通性检验，输出 `P2_demo.png`）；
