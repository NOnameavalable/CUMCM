...
本文档对 [`P2.py`](file:///Users/ray_zhang/Developer/CUMCM/P2.py) 模块进行系统性、全覆盖的逐行原理解读。旨在帮助读者彻底理解**题目背景、物理模型、数学本质、架构逻辑**，并对代码中**每一个数据类、每一个函数（共 43 个函数，无遗漏）**的输入、输出、处理逻辑和设计用意做出深入透彻的解释。
...
4. [逐函数深度拆解与原理解读（全函数覆盖）](#4-逐函数深度拆解与原理解读全函数覆盖)
   ...
   - [4.4 候选点搜索空间与单侧对称采样模块（4个函数）](#44-候选点搜索空间与单侧对称采样模块)
...
- **每一步在干什么**：
  ...
  3. **横向坐标 $b$（侧向偏移）**：**严格将下界固定为 $b_{\min} = 0.0$**，上界延伸至 $\max(\max(b) + 300\text{ m}, 1000\text{ m})$。
  4. **设计用意（对称性降维）**：由于第一条示向线两侧的物理环境在统计上完全轴对称，候选点 $(a, b)$ 与 $(a, -b)$ 的交会几何精度完全一致。强制在单半平面（$b \ge 0$）采样可使算力集中，并将全部局部细筛种子投入优势区域。
...
### Q2: 为什么可以只在 $b \ge 0$ 的单侧空间进行搜索？
- **物理镜像对称性**：在初始全向辐射且无定向遮挡的情境下，以第一次示向线为对称轴，左右两侧的几何关系与误差概率是对偶的。
- 若两侧同时搜索，由于粗细网格离散采样的舍入微差，往往会出现一边比另一边“偶然好 $0.001\text{m}$”的数值假象，并将有限的细筛种子分散在两侧。
- 采用单半平面约束，将全部 8 个细筛种子全力投入单侧进行高密度挖掘，不仅求解效率提升 1 倍，解的几何连续性也更佳。最终结果只需通过 $(a, -b)$ 即可镜像得到对称区域。
